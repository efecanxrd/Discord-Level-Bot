...
module.exports = {
    ...
    async execute (message, args) {
        ...
        const image = './img/unknown.png'
        const color = config.textandProgressBarColor
    const rankCard = new canvacord.Rank()
        ...
        .setDiscriminator('5555')
        .setRank(ranking)
        .setRankColor(color)
        .setLevel(levelInfo)
        .setLevelColor(color)
        .setAvatarColor(config.avatarColor)
        .setXpTextColor(config.xpTextColor)
        .setUsernameColor(config.usernameColor)
        .setRankAndLevelTextColor(config.RankAndLevelTextColor)
        rankCard.data.overlay.display = false;
        rankCard.build()
    .then(data => {
